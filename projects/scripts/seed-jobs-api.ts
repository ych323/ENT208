/**
 * 岗位数据填充脚本
 * 通过调用 /api/jobs/list POST 接口批量导入岗位数据
 * 
 * 使用方法: pnpm tsx scripts/seed-jobs-api.ts
 */
...
async function seedJobs() {
  ...
  console.log(`共 ${JOBS_DATA.length} 条岗位数据`);
  ...
  for (const job of JOBS_DATA) {
    try {
      ...
      if (response.ok) {
        ...
        errorCount++;
        console.log(`✗ ${job.company} - ${job.title} (${response.status})`);
      }
    } catch (error) {
      errorCount++;
      console.log(`✗ ${job.company} - ${job.title} (Network error)`);
    }
  }
  
  console.log(`\n填充完成! 成功: ${successCount}, 失败: ${errorCount}`);
}
